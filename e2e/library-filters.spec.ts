...
for (const route of ['/', '/library-status']) {
  test(`${route} exposes matching title and status filters`, async ({ page }) => {
    ...
    // The isolated fixture has one unfiled song; title filtering must not remove
    // the table's instrument columns or change any catalog metadata.
    const search = page.getByRole('searchbox', { name: 'Search titles' });
    await search.fill('  EXAMPLE  ');
    await expect(page.locator('.count').first()).toContainText('1 of 1');
    await filters.getByRole('button', { name: 'Unfiled', exact: true }).click();
    await expect(page.locator('.count').first()).toContainText('0 of 1');
